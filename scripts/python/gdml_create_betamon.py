...
def _fmt(value: float) -> str:
    """Format floats for GDML XML attributes with stable, compact precision."""
    text = f"{float(value):.12g}"
    return "0" if text == "-0" else text


def _sub(parent: ET.Element, tag: str, **attrs) -> ET.Element:
    """Create a child XML element and normalize any float attributes."""
...
def _position(parent: ET.Element, name: str, xyz: tuple[float, float, float]) -> ET.Element:
    """Write a GDML position element in millimeters."""
...
def _rotation(parent: ET.Element, name: str, xyz: tuple[float, float, float]) -> ET.Element:
    """Write a GDML rotation element in degrees."""
...
def volume_spec(
    ...
    *,
    ...
    """Build a compact volume description for later GDML emission."""
    return VolumeSpec(
        name=name,
        material=material,
        solid=solid,
...
class GeometryExportBuilder:
    """Assemble a complete GDML document from geometry and material metadata."""

    def __init__(self, params: GeometryParams):
        self.p = params

    def write(self, output_path: str | Path) -> Path:
        """Create the GDML tree, pretty-print it, and write it to disk."""
    ...
    def build(self) -> ET.Element:
        """Create the root GDML tree in the order Geant4 expects."""
        root = ET.Element(
            "gdml",
            {
                "xmlns:xsi": GDML_XSI,
    ...
    def _emit_materials(self, root: ET.Element) -> None:
        """Emit all isotopes, elements, and materials used by the geometry."""
        materials = ET.SubElement(root, "materials")

        def isotope(name: str, n: int, z: int, mass: float) -> None:
            # Each isotope is emitted as a reusable leaf node, then referenced by elements.
        ...
        def element(name: str, fractions: tuple[tuple[float, str], ...]) -> None:
            # Elements point at isotope definitions through fractional abundance entries.
        ...
        # Natural elements used in compound materials.
    ...
    def _emit_polycone(
        ...
        solids: ET.Element,
        name: str,
        zplanes: tuple[tuple[float, float, float], ...],
        *,
        startphi: float = 0.0,
        deltaphi: float = 360.0,
    ) -> None:
        """Emit a GDML polycone solid from a sequence of z-plane definitions."""
        node = _sub(solids, "polycone", name=name, aunit="deg", deltaphi=deltaphi, lunit="mm", startphi=startphi)
        for rmax, rmin, z in zplanes:
            _sub(node, "zplane", rmax=rmax, rmin=rmin, z=z)
    ...
    def _emit_union(self, solids: ET.Element, name: str, first: str, second: str, pos: tuple[float, float, float], rot: tuple[float, float, float] | None = None) -> None:
        """Emit a GDML union solid and its placement transform."""
    ...
    def _emit_subtraction(self, solids: ET.Element, name: str, first: str, second: str, pos: tuple[float, float, float] | None = None, rot: tuple[float, float, float] | None = None) -> None:
        """Emit a GDML subtraction solid and optional transform."""
    ...
    def _emit_solids(self, solids: ET.Element) -> None:
        """Emit the full set of solids in the same order used by the reference GDML."""
        p = self.p

        # Inner and outer T-pipe pieces are assembled from two orthogonal tubes.
        ...
        # Flanges and the thin aluminum window around the pipe opening.
        ...
        # Revolved wedge/ring used near the window region.
        self._emit_polycone(solids, "WedgeRevolvedRing", p.wedge_zplanes)

        # Scintillator A stack is emitted as the core volume plus its wrapped layers.
        self._emit_box(solids, "AScintCore", p.scint_a_xy, p.scint_a_xy, p.scint_a_z)
        self._emit_box(solids, "AScintInnerGeom", p.scint_a_xy + p.scint_a_inner_al_delta, p.scint_a_xy + p.scint_a_inner_al_delta, p.scint_a_z + p.scint_a_inner_al_delta)
        self._emit_subtraction(solids, "AScintInnerAl", "AScintInnerGeom", "AScintCore")
        self._emit_box(solids, "AScintMylarGeom", p.scint_a_xy + p.scint_a_mylar_delta, p.scint_a_xy + p.scint_a_mylar_delta, p.scint_a_z + p.scint_a_mylar_delta)
        self._emit_subtraction(solids, "AScintMylar", "AScintMylarGeom", "AScintInnerGeom")
        self._emit_box(solids, "AScintOuterGeom", p.scint_a_xy + p.scint_a_outer_al_delta, p.scint_a_xy + p.scint_a_outer_al_delta, p.scint_a_z + p.scint_a_outer_al_delta)
        self._emit_subtraction(solids, "AScintOuterAl", "AScintOuterGeom", "AScintMylarGeom")

        # Scintillator B stack mirrors A but is placed farther from the window.
        self._emit_box(solids, "BScintCore", p.scint_b_xy, p.scint_b_xy, p.scint_b_z)
        self._emit_box(solids, "BScintInnerGeom", p.scint_b_xy + p.scint_b_inner_al_delta, p.scint_b_xy + p.scint_b_inner_al_delta, p.scint_b_z + p.scint_b_inner_al_delta)
        self._emit_subtraction(solids, "BScintInnerAl", "BScintInnerGeom", "BScintCore")
        self._emit_box(solids, "BScintMylarGeom", p.scint_b_xy + p.scint_b_mylar_delta, p.scint_b_xy + p.scint_b_mylar_delta, p.scint_b_z + p.scint_b_mylar_delta)
        self._emit_subtraction(solids, "BScintMylar", "BScintMylarGeom", "BScintInnerGeom")
        self._emit_box(solids, "BScintOuterGeom", p.scint_b_xy + p.scint_b_outer_al_delta, p.scint_b_xy + p.scint_b_outer_al_delta, p.scint_b_z + p.scint_b_outer_al_delta)
        self._emit_subtraction(solids, "BScintOuterAl", "BScintOuterGeom", "BScintMylarGeom")

        # Rear mechanical pieces and the source assembly.
        ...
        # World solid is emitted last so the structure section can reference it cleanly.
        self._emit_box(solids, "World", *p.world_xyz)

    def _volume_specs(self) -> tuple[VolumeSpec, ...]:
        """Describe every placed volume, including material, solid, and transform."""
        p = self.p
        return (
    ...
    def _emit_structure(self, root: ET.Element) -> None:
        """Emit the placed volumes and wire them into the world hierarchy."""
        ...
        world = nodes["World"]
        # Attach each placed volume to the world with its own GDML physvol entry.
        for spec in specs:
            if spec.name == "World":
                continue
...
def build_geometry(output_path: str | Path) -> Path:
    """Convenience wrapper for generating a GDML file at a chosen output path."""
    return GeometryExportBuilder(PARAMS).write(output_path)


def main(argv: Sequence[str] | None = None) -> int:
    """Parse CLI arguments and write the requested GDML export to disk."""
